...
export default function Orders() {
  ...
  useEffect(() => {
    try {
      const savedOrders = JSON.parse(localStorage.getItem('forever-orders')) || []
    ...
    <Layout>
      <div className="page-container min-h-[900px] pt-16 sm:min-h-[1500px] sm:pt-20 lg:min-h-[2100px] lg:pt-24">
        ...
        <div className="animate-enter-delay mt-8 overflow-hidden rounded-2xl border border-line bg-ivory/60 shadow-[0_8px_24px_rgba(67,54,38,0.035)]">
          {orders.length ? (
            orders.map((order) => {
              const prodImage = Array.isArray(order.product?.image)
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-3 text-[12px] text-[#625b54] sm:text-[13px]">
                      {currency}{order.product?.price} &nbsp; Quantity: {order.quantity || 1} &nbsp; Size: {order.size || 'L'}
                    </p>
                    <p className="mt-4 text-[11px] text-muted sm:text-[12px]">Date: {order.date}</p>
                  </div>
            ...
            <div className="py-20 text-center text-sm text-muted">No orders placed yet.</div>
          )}
        </div>
      </div>
    </Layout>
